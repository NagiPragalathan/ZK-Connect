...
def home(request):
    if not request.user.is_authenticated:
        ...
        if request.user.is_staff:
            return render(request, "new_temp/index.html", {'staff': "staff",'req':OBJ})
        else:
            return render(request, "new_temp/index.html",{'req':OBJ})
def contact(request):
    return render(request,"new_temp/contact.html")
def job_details(request,post_id):
    # usr=User.objects.get(id=post_id)
    obj = JobPost.objects.get(company_id=post_id)
    company = CompanyDetails.objects.get(company_id=obj.company_id)
    # print("comapny : ",company,company.company_logo.url)
    if not request.user.is_authenticated:
        return render(request,"new_temp/job_details.html",{'datas':obj,'company':company,'msg':'no_login'})
    else:
        return render(request,"new_temp/job_details.html",{'datas':obj,'company':company})
    
def job_listing(request):
    if not request.user.is_authenticated:
        return render(request,"new_temp/job_listing.html",{'msg':'no_login'})
    else:
        return render(request,"new_temp/job_listing.html")
